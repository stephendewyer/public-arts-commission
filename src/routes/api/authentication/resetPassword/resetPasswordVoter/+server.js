...
export async function POST({request}) {
    ...
    // get the user from users_voters table in database
    ...
    if (voter.length === 0) {
        ...
        return new Response(JSON.stringify({error: `no voter account with ${email} as email exists!`}), {status: 422});
    ...
    // add the token and token expiration date to the user row in users_voters table
    const tokenUpdateStatement = `UPDATE users_voters
    SET 
        reset_token = "${token}", 
    ...
    const msg = {
        ...
        html: `
            ...
            <p>Click <a href="${DOMAIN}/update-password/voter/${token}" >this link to set a new password.</a></p>
            <p>Kind regards,</p>
