<script lang="ts">
	...
	import { createEndorsedCandidatesSearchStore } from '$lib/stores/EndorsedCandidatesSearchStore.js';
	...
	const endorsementsType: string = "candidates";

    // get the endorsed candidates

	let endorsedCandidates = $state<CandidateWithImage[]>([]);

	let pendingEndorsedCandidatesData: boolean | null = $state(null);

	let getEndorsedCandidatesDataSuccess: boolean | null = $state(null);

	async function getEndorsedCandidates() {
		...
		// order candidates by most recent election_data
    ...
    const searchEndorsedCandidates: SearchEndorsedCandidateWithImage[] = $derived.by(() => {
        ...
        return endorsedCandidates.map((candidate: CandidateWithImage) => {
            ...
            return {
                ...
                searchTerms: {
					...
					in_active_election: candidate.running_in_primary || candidate.running_in_general ? true : false,
					in_active_primary_election: candidate.running_in_primary ? true : false,
					in_active_general_election: candidate.rejected_in_general ? true : false,
					elected_into_office: candidate.elected_in_general ? true : false,
					nonincumbent: candidate.incumbent ? false : true
                }
            };
    ...
    // create the endorsedCandidatesSearchStore

    const endorsedCandidatesSearchStore = createEndorsedCandidatesSearchStore();

    // filter endorsedCandidatesSearchStore data using createEndorsedCandidatesSearchStore

    interface FilteredEndorsedCandidates {
        all: SearchEndorsedCandidateWithImage[];
		federal: SearchEndorsedCandidateWithImage[];
		state: SearchEndorsedCandidateWithImage[];
		county: SearchEndorsedCandidateWithImage[];
		city: SearchEndorsedCandidateWithImage[];
	};

    const filteredEndorsedCandidates: FilteredEndorsedCandidates = $derived.by(() => {

        const filteredCandidates: SearchEndorsedCandidateWithImage[] =  searchEndorsedCandidates.filter((candidate) => {

            const searchYear = $endorsedCandidatesSearchStore.year.toString();

            // if search by year is empty, continue with search by street address

            if (searchYear === "") {

                return SearchEndorsementsByStreetAddressFilter($endorsedCandidatesSearchStore, candidate);

            // else if search year matches item searchTerm year, continue with filters

            } else if (candidate.searchTerms.year === searchYear) {
        ...
        // sort the candidates by government level

        let all: SearchEndorsedCandidateWithImage[] = [];
        let federal: SearchEndorsedCandidateWithImage[] = [];
        let state: SearchEndorsedCandidateWithImage[] = [];
        let county: SearchEndorsedCandidateWithImage[] = [];
        let city: SearchEndorsedCandidateWithImage[] = [];

        filteredCandidates.forEach((candidate: SearchEndorsedCandidateWithImage) => {
            all = [...all, candidate];

            if (candidate.government_level === "federal") {
                federal = [...federal, candidate];
            } else if (candidate.government_level === "state") {
                state = [...state, candidate];
            } else if (candidate.government_level === "county") {
                county = [...county, candidate];
	...
	let activeElectionsOnlyChecked: boolean = $state(false);
	let activeGeneralElectionsOnlyChecked: boolean = $state(false);
	let activePrimaryElectionsOnlyChecked: boolean = $state(false);
	let electedIntoOfficeOnlyChecked: boolean = $state(false);
	let nonincumbentOnlyChecked: boolean = $state(false);
	...
	async function reverseGeocode(latitude: number | null, longitude: number | null): Promise<string | undefined> {
		...
		if (response.ok) {
			...
			endorsedCandidatesSearchStore.setSearch({
				...
				city: location.city,
				name: name,
				in_active_election: activeElectionsOnlyChecked,
				in_active_primary_election: activePrimaryElectionsOnlyChecked,
				in_active_general_election: activeGeneralElectionsOnlyChecked,
				elected_into_office: electedIntoOfficeOnlyChecked,
				nonincumbent: nonincumbentOnlyChecked
			});
	...
	let endorsementTabPanels: EndorsementTabPanel[] = $derived([
		{
			...
			label: "all",
			hasCapitol: true,
			data: filteredEndorsedCandidates.all,
		},
		{
			id: uuidv4(),
			index: 1,
			label: "federal",
			hasCapitol: true,
			data: filteredEndorsedCandidates.federal,
		},
		{
			id: uuidv4(),
			index: 2,
			label: "state",
			hasCapitol: true,
			data: filteredEndorsedCandidates.state,
		},
		{
			id: uuidv4(),
			index: 3,
			label: "county",
			hasCapitol: true,
			data: filteredEndorsedCandidates.county,
		},
		{
			id: uuidv4(),
			index: 4,
			label: "city",
			hasCapitol: true,
			data: filteredEndorsedCandidates.city,
		},
	]);
...
<svelte:head>
	<title>endorsed candidates - public arts commission</title>
	<meta name="description" content="find public arts commission-endorsed candidates" />
