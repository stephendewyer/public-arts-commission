<script lang="ts">
	...
	const endorsementsType: string = "legislation";

    // get the endorsed legislation

	let endorsedLegislation = $state<SearchLegislationWithSponsorsAndImage[]>([]);

	let pendingEndorsedLegislationData: boolean | null = $state(null);

	let getEndorsedLegislationDataSuccess: boolean | null = $state(null);

	async function getEndorsedLegislation() {
		...
		// order legislation by year released

		if (response.ok) {
			endorsedLegislation.sort((a: number | any, b: number | any) => {
			...
			pendingEndorsedLegislationData = false;
			getEndorsedLegislationDataSuccess = false;
		};
    ...
    const searchEndorsedLegislation: SearchLegislationWithSponsorsAndImage[] = $derived.by(() => {
        ...
        return endorsedLegislation.map((legislation: SearchLegislationWithSponsorsAndImage) => {

            // get the House and/or Senate introduction dates for year

            const yearIntroducedInHouse: string = legislation.year_introduced_House ? legislation.year_introduced_House.toString() : "";
            const yearIntroducedInSenate: string = legislation.year_introduced_Senate ? legislation.year_introduced_Senate.toString() : "";

            const yearIntroduced: string[] | never [] = [ yearIntroducedInHouse, yearIntroducedInSenate ];
            ...
            return {
                ...
                searchTerms: {
                    ...
                    sponsors: [...legislation.sponsors_House, ...legislation.sponsors_Senate],
                    cosponsors: [...legislation.co_sponsors_House, ...legislation.co_sponsors_Senate],
                    introduced_in_Senate_only: legislation.year_introduced_Senate ? true : false,
                    introduced_in_House_only: legislation.year_introduced_House ? true : false,
                    scheduled_for_vote_in_House_only: legislation.scheduled_vote_House ? true : false,
                    scheduled_for_vote_in_Senate_only: legislation.scheduled_vote_Senate ? true : false,
                    approved_in_House_only: legislation.passed_in_House ? true : false,
                    approved_in_Senate_only: legislation.passed_in_Senate ? true : false,
                    signed_into_law_by_Executive_only: legislation.signed_by_Executive ? true : false,
                    vetoed_by_Executive_only: legislation.vetoed_by_Executive ? true : false,
                    overridden_Executive_veto_only: legislation.overridden_Executive_veto ? true : false
                }
            };
    ...
    // create the endorsedLegislationSearchStore

    const endorsedLegislationSearchStore = createEndorsedLegislationSearchStore();
    ...
    interface FilteredEndorsedLegislation {
        all: SearchLegislationWithSponsorsAndImage[];
		federal: SearchLegislationWithSponsorsAndImage[];
		state: SearchLegislationWithSponsorsAndImage[];
		county: SearchLegislationWithSponsorsAndImage[];
		city: SearchLegislationWithSponsorsAndImage[];
	};

    const filteredEndorsedLegislation: FilteredEndorsedLegislation = $derived.by(() => {

        const filteredLegislation: SearchLegislationWithSponsorsAndImage[] =  searchEndorsedLegislation.filter((legislation) => {

            const searchYear: string[] = $endorsedLegislationSearchStore.year;

            // if search by year is empty, continue with search by street address

            if (searchYear.length === 0) {

                return SearchEndorsementsByStreetAddressFilter($endorsedLegislationSearchStore, legislation);

            // else if search year matches item searchTerm year, continue with filters

            } else if (legislation.searchTerms.year.join(", ").includes(searchYear[0])) {
        ...
        // sort the legislation by government level

        let all: SearchLegislationWithSponsorsAndImage[] = [];
        let federal: SearchLegislationWithSponsorsAndImage[] = [];
        let state: SearchLegislationWithSponsorsAndImage[] = [];
        let county: SearchLegislationWithSponsorsAndImage[] = [];
        let city: SearchLegislationWithSponsorsAndImage[] = [];

        filteredLegislation.forEach((legislation: SearchLegislationWithSponsorsAndImage) => {
            all = [...all, legislation];
            ...
            if (legislation.government_level === "federal") {
                ...
                county = [...county, legislation];
            } else if (legislation.government_level === "city") {
                city = [...city, legislation];
            };
    ...
    let sponsorsInputValue: string[] = $state([]);
    let introducedSenateOnlyChecked: boolean = $state(false);
    let introducedHouseOnlyChecked: boolean = $state(false);
    let voteScheduledHouseOnlyChecked: boolean = $state(false);
    let voteScheduledSenateOnlyChecked: boolean = $state(false);
    let houseApprovedOnlyChecked: boolean = $state(false);
    let senateApprovedOnlyChecked: boolean = $state(false);
    let signedExecutiveLawChecked: boolean = $state(false);
    let vetoedByExecutiveChecked: boolean = $state(false);
    let executiveOverriddenChecked: boolean = $state(false);
	...
	async function reverseGeocode(latitude: number | null, longitude: number | null): Promise<string | undefined> {
		...
		if (response.ok) {
			...
			endorsedLegislationSearchStore.setSearch({
				...
				city: location.city,
				name: name,
                sponsors: sponsorsInputValue,
                cosponsors: sponsorsInputValue,
                introduced_in_Senate_only: introducedSenateOnlyChecked,
                introduced_in_House_only: introducedHouseOnlyChecked,
                scheduled_for_vote_in_House_only: voteScheduledHouseOnlyChecked,
                scheduled_for_vote_in_Senate_only: voteScheduledSenateOnlyChecked,
                approved_in_House_only: houseApprovedOnlyChecked,
                approved_in_Senate_only: senateApprovedOnlyChecked,
                signed_into_law_by_Executive_only: signedExecutiveLawChecked,
                vetoed_by_Executive_only: vetoedByExecutiveChecked,
                overridden_Executive_veto_only: executiveOverriddenChecked
			});
	...
	let endorsementTabPanels: EndorsementTabPanel[] = $derived([
		{
			...
			label: "all",
			hasCapitol: true,
			data: filteredEndorsedLegislation.all,
		},
		{
			id: uuidv4(),
			index: 1,
			label: "federal",
			hasCapitol: true,
			data: filteredEndorsedLegislation.federal,
		},
		{
			id: uuidv4(),
			index: 2,
			label: "state",
			hasCapitol: true,
			data: filteredEndorsedLegislation.state,
		},
		{
			id: uuidv4(),
			index: 3,
			label: "county",
			hasCapitol: true,
			data: filteredEndorsedLegislation.county,
		},
		{
			id: uuidv4(),
			index: 4,
			label: "city",
			hasCapitol: true,
			data: filteredEndorsedLegislation.city,
		},
	]);
...
<svelte:head>
	<title>endorsed legislation - public arts commission</title>
	<meta name="description" content="find public arts commission-endorsed legislation" />
	...
	>
		...
		<div class="results_container">
			<div 
				...
				class="results"
			>
				{#if pendingEndorsedLegislationData}
					<LoaderAnimation />
				{:else if getEndorsedLegislationDataSuccess}
					<Panel 
