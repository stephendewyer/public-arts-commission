<script lang="ts">
	...
	const endorsementsType: string = "amendments";

    // get the endorsed amendments

	let endorsedAmendments = $state<AmendmentWithSponsorsAndImage[]>([]);

	let pendingEndorsedAmendmentsData: boolean | null = $state(null);

	let getEndorsedAmendmentsDataSuccess: boolean | null = $state(null);

	async function getEndorsedAmendments() {
		...
		if (response.ok) {
			// order amendments by name

			endorsedAmendments.sort((a: AmendmentWithSponsorsAndImage, b: AmendmentWithSponsorsAndImage) => a.amendment_name.localeCompare(b.amendment_name));
	...
	// handle opening amendments drawer on page load with search params
	...
	// handle opening amendment drawer after page load

	$effect(() => {
		if (!getEndorsedAmendmentsDataSuccess) return;

		const amendmentID = page.url.searchParams.get("amendment_ID");
		...
		const amendment = searchEndorsedAmendments.find(
			a => a.amendment_ID.toString() === amendmentID
		);
    ...
    const searchEndorsedAmendments: SearchAmendmentWithSponsorsAndImage[] = $derived.by(() => {

        return endorsedAmendments.map((amendment: AmendmentWithSponsorsAndImage) => {

			const yearIntroducedHouse: string = amendment.year_introduced_House.toString() ? amendment.year_introduced_House.toString() : "";
			const yearIntroducedSenate: string = amendment.year_introduced_Senate.toString() ? amendment.year_introduced_Senate.toString() : "";
			const electionYear: string = amendment.election_date ? new Date(amendment.election_date).getFullYear().toString() : "";
			const year = [yearIntroducedHouse, yearIntroducedSenate, electionYear];
            ...
            return {
                ...
                searchTerms: {
					...
					sponsors: [...amendment.sponsors_House, ...amendment.sponsors_Senate],
					cosponsors: [...amendment.co_sponsors_House, ...amendment.co_sponsors_Senate],
					introduced_in_House: amendment.introduced_in_House ? true : false,
					introduced_in_Senate: amendment.introduced_in_Senate ? true : false,
					twothirds_House_and_Senate_passed: amendment.twothirds_House_and_Senate_passed ? true : false,
					simple_majority_House_and_Senate_passed: amendment.simple_majority_House_and_Senate_passed ? true : false,
					simple_majority_voters_passed: amendment.simple_majority_voters_passed ? true : false,
					ratified_by_state_conventions: amendment.ratified_by_state_conventions ? true : false,
					ratified_by_state_legislatures: amendment.ratified_by_state_legislatures ? true : false,
					scheduled_for_vote_in_House: amendment.scheduled_for_vote_in_House ? true : false,
					scheduled_for_vote_in_Senate: amendment.scheduled_for_vote_in_Senate ? true : false,
					enacted_into_law: amendment.enacted_into_law ? true : false,
					on_ballot_for_election_by_voters: amendment.on_ballot_for_election_by_voters ? true : false,
					approved_by_voters: amendment.approved_by_voters ? true : false
                }
            };
    ...
    // create the endorsedAmendmentsSearchStore

    const endorsedAmendmentsSearchStore = createEndorsedAmendmentsSearchStore();

    // filter endorsedAmendmentsSearchStore data using createEndorsedAmendmentsSearchStore

    interface FilteredEndorsedAmendments {
        all: SearchAmendmentWithSponsorsAndImage[];
		federal: SearchAmendmentWithSponsorsAndImage[];
		state: SearchAmendmentWithSponsorsAndImage[];
		county: SearchAmendmentWithSponsorsAndImage[];
		city: SearchAmendmentWithSponsorsAndImage[];
	};

    const filteredEndorsedAmendments: FilteredEndorsedAmendments = $derived.by(() => {

        const filteredAmendments: SearchAmendmentWithSponsorsAndImage[] =  searchEndorsedAmendments.filter((amendment) => {

            const searchYear: string[] = $endorsedAmendmentsSearchStore.year;

            // if search by year is empty, continue with search by street address

            if (searchYear.length === 0) {

                return SearchEndorsementsByStreetAddressFilter($endorsedAmendmentsSearchStore, amendment);

            // else if search year matches item searchTerm year, continue with filters

            } else if (amendment.searchTerms.year.join(", ").includes(searchYear[0])) {
        ...
        let all: SearchAmendmentWithSponsorsAndImage[] = [];
        let federal: SearchAmendmentWithSponsorsAndImage[] = [];
        let state: SearchAmendmentWithSponsorsAndImage[] = [];
        let county: SearchAmendmentWithSponsorsAndImage[] = [];
        let city: SearchAmendmentWithSponsorsAndImage[] = [];

        filteredAmendments.forEach((amendment: SearchAmendmentWithSponsorsAndImage) => {
            all = [...all, amendment];

            if (amendment.government_level === "federal") {
                federal = [...federal, amendment];
            } else if (amendment.government_level === "state") {
                state = [...state, amendment];
            } else if (amendment.government_level === "county") {
                county = [...county, amendment];
	...
	let sponsorsValue: string[] = $state([]);
	let introducedHouseChecked: boolean = $state(false);
	let introducedSenateChecked: boolean = $state(false);
	let twoThirdsHouseSenatePassedChecked: boolean = $state(false);
	let majorityHouseSenatePassedChecked: boolean = $state(false);
	let ratifiedStateConventionsChecked: boolean = $state(false);
	let ratifiedStateLegislaturesChecked: boolean = $state(false);
	let scheduledHouseVoteChecked: boolean = $state(false);
	let scheduledSenateVoteChecked: boolean = $state(false);
	let enactedLawChecked: boolean = $state(false);
	let pendingVoterElectionChecked: boolean = $state(false);
	let votersApprovedChecked: boolean = $state(false);
	...
	async function reverseGeocode(latitude: number | null, longitude: number | null): Promise<string | undefined> {
		...
		if (response.ok) {
			...
			endorsedAmendmentsSearchStore.setSearch({
				...
				cosponsors: sponsorsValue,
				introduced_in_House: introducedHouseChecked,
				introduced_in_Senate: introducedSenateChecked,
				twothirds_House_and_Senate_passed: twoThirdsHouseSenatePassedChecked,
				simple_majority_House_and_Senate_passed: majorityHouseSenatePassedChecked,
				simple_majority_voters_passed: votersApprovedChecked,
				ratified_by_state_conventions: ratifiedStateConventionsChecked,
				ratified_by_state_legislatures: ratifiedStateLegislaturesChecked,
				scheduled_for_vote_in_House: scheduledHouseVoteChecked,
				scheduled_for_vote_in_Senate: scheduledSenateVoteChecked,
				enacted_into_law: enactedLawChecked,
				on_ballot_for_election_by_voters: pendingVoterElectionChecked,
				approved_by_voters: votersApprovedChecked
			});
	...
	let endorsementTabPanels: EndorsementTabPanel[] = $derived([
		{
			...
			label: "all",
			hasCapitol: true,
			data: filteredEndorsedAmendments.all,
		},
		{
			id: uuidv4(),
			index: 1,
			label: "federal",
			hasCapitol: true,
			data: filteredEndorsedAmendments.federal,
		},
		{
			id: uuidv4(),
			index: 2,
			label: "state",
			hasCapitol: true,
			data: filteredEndorsedAmendments.state,
		},
		{
			id: uuidv4(),
			index: 3,
			label: "county",
			hasCapitol: true,
			data: filteredEndorsedAmendments.county,
		},
		{
			id: uuidv4(),
			index: 4,
			label: "city",
			hasCapitol: true,
			data: filteredEndorsedAmendments.city,
		},
	]);
...
<svelte:head>
	<title>endorsed amendments - public arts commission</title>
