<script lang="ts">
    ...
    let endorsedAmendmentImagesAndSponsors: AmendmentWithSponsorsAndImage = data.streamed.endorsedAmendmentImagesAndSponsors;

    let coSponsorsHouseValues: CoSponsorInputValue[] = [];

    if (endorsedAmendmentImagesAndSponsors.co_sponsors_House.length > 0) {
        endorsedAmendmentImagesAndSponsors.co_sponsors_House?.forEach((sponsor: CoSponsorHouse) => {
    ...
    if (endorsedAmendmentImagesAndSponsors.co_sponsors_Senate.length > 0) {
        endorsedAmendmentImagesAndSponsors.co_sponsors_Senate?.forEach((sponsor: CoSponsorSenate) => {
    ...
    let imageAltTextInputValue: string = endorsedAmendmentImagesAndSponsors?.alt_text ? reverseHtmlEntities(endorsedAmendmentImagesAndSponsors?.alt_text) : "";
    let image: any | string = endorsedAmendmentImagesAndSponsors?.image_URL ? endorsedAmendmentImagesAndSponsors?.image_URL : "";
    let imageID: number | null = endorsedAmendmentImagesAndSponsors?.image_ID ? endorsedAmendmentImagesAndSponsors?.image_ID : null;
    let imagePublicID: string = endorsedAmendmentImagesAndSponsors?.public_ID ? endorsedAmendmentImagesAndSponsors?.public_ID : "";
    let amendmentNameInputValue: string = endorsedAmendmentImagesAndSponsors?.amendment_name ? reverseHtmlEntities(endorsedAmendmentImagesAndSponsors?.amendment_name) : "";
    let yearReleasedInputValue: number | string = endorsedAmendmentImagesAndSponsors?.year_released ? endorsedAmendmentImagesAndSponsors?.year_released : "";
    let yearIntroducedInHouseInputValue: number | string = endorsedAmendmentImagesAndSponsors?.year_introduced_House ? endorsedAmendmentImagesAndSponsors?.year_introduced_House : "";
    let yearIntroducedInSenateInputValue: number | string = endorsedAmendmentImagesAndSponsors?.year_introduced_Senate ? endorsedAmendmentImagesAndSponsors?.year_introduced_Senate : "";
    let electionDateInputValue: string = endorsedAmendmentImagesAndSponsors?.election_date ? ConvertDateInputFormat(new Date(endorsedAmendmentImagesAndSponsors?.election_date)) : "";
    let governmentLevelInputValue: string = endorsedAmendmentImagesAndSponsors?.government_level ? endorsedAmendmentImagesAndSponsors?.government_level : "";
    let stateInputValue: string = endorsedAmendmentImagesAndSponsors?.state ? endorsedAmendmentImagesAndSponsors?.state : "";
    let countyInputValue: string = endorsedAmendmentImagesAndSponsors?.county ? endorsedAmendmentImagesAndSponsors?.county : "";
    let cityInputValue: string = endorsedAmendmentImagesAndSponsors?.city ? endorsedAmendmentImagesAndSponsors?.city : "";
    let introducedInHouseChecked: boolean | any = endorsedAmendmentImagesAndSponsors?.introduced_in_House;
    let introducedInSenateChecked: boolean | any = endorsedAmendmentImagesAndSponsors?.introduced_in_Senate;
    let sponsorHouseInputValue: string = endorsedAmendmentImagesAndSponsors?.sponsors_House[0]?.sponsor_name ? endorsedAmendmentImagesAndSponsors?.sponsors_House[0]?.sponsor_name : "";
    let sponsorSenateInputValue: string = endorsedAmendmentImagesAndSponsors?.sponsors_Senate[0]?.sponsor_name ? endorsedAmendmentImagesAndSponsors?.sponsors_Senate[0]?.sponsor_name : "";
    let houseSessionInputValue: string = endorsedAmendmentImagesAndSponsors?.session_House ? endorsedAmendmentImagesAndSponsors?.session_House : "";
    let senateSessionInputValue: string = endorsedAmendmentImagesAndSponsors?.session_Senate ? endorsedAmendmentImagesAndSponsors?.session_Senate : "";
    let detailsInputValue: string = endorsedAmendmentImagesAndSponsors?.details ? reverseHtmlEntities(endorsedAmendmentImagesAndSponsors?.details) : "";
    let websiteURLInputValue: string = endorsedAmendmentImagesAndSponsors?.website_URL ? endorsedAmendmentImagesAndSponsors?.website_URL : "";
    let twoThirdsHouseAndSenatePassedChecked: boolean | any = endorsedAmendmentImagesAndSponsors?.twothirds_House_and_Senate_passed;
    let simpleMajorityHouseAndSenatePassedChecked: boolean | any = endorsedAmendmentImagesAndSponsors?.simple_majority_House_and_Senate_passed;
    let simpleMajorityVotersPassedChecked: boolean | any = endorsedAmendmentImagesAndSponsors?.simple_majority_voters_passed;
    let ratifiedByStateLegislaturesChecked: boolean | any = endorsedAmendmentImagesAndSponsors?.ratified_by_state_legislatures;
    let ratifiedByStateConventionsChecked: boolean | any = endorsedAmendmentImagesAndSponsors?.ratified_by_state_convenctions;

    let nameFirstContactInputValue: string = endorsedAmendmentImagesAndSponsors?.contact_name_first ? endorsedAmendmentImagesAndSponsors?.contact_name_first : "";
    let nameLastContactInputValue: string = endorsedAmendmentImagesAndSponsors?.contact_name_last ? endorsedAmendmentImagesAndSponsors?.contact_name_last : "";
    let phoneContactInputValue: E164Number | string = endorsedAmendmentImagesAndSponsors?.contact_phone_number ? endorsedAmendmentImagesAndSponsors?.contact_phone_number : "";
    let streetAddressContactInputValue: string = endorsedAmendmentImagesAndSponsors?.contact_street_address ? endorsedAmendmentImagesAndSponsors?.contact_street_address : "";
    let streetAddress02ContactInputValue: string = endorsedAmendmentImagesAndSponsors?.contact_street_address_02 ? endorsedAmendmentImagesAndSponsors?.contact_street_address_02 : "";
    let cityContactInputValue: string = endorsedAmendmentImagesAndSponsors?.contact_city ? endorsedAmendmentImagesAndSponsors?.contact_city : "";
    let stateContactInputValue: string = endorsedAmendmentImagesAndSponsors?.contact_state ? endorsedAmendmentImagesAndSponsors?.contact_state : "";
    let zipCodeContactInputValue: number | string = endorsedAmendmentImagesAndSponsors?.contact_zip_code ? endorsedAmendmentImagesAndSponsors?.contact_zip_code : "";
    let emailContactInputValue: string = endorsedAmendmentImagesAndSponsors?.contact_email ? endorsedAmendmentImagesAndSponsors?.contact_email : "";
    ...
    const submitAmendmentEndoresementHandler = async () => {
        ...
        try {
            ...
            await updateAmendmentEndorsement(
                userEmail,
                endorsedAmendmentImagesAndSponsors.amendment_ID,
                imageFileInputValue,
                imagePublicID,
...
<svelte:head>
	...
	<meta name="description" content="edit an amendment endorsement by public arts commission" />
